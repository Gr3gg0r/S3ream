...
- The landing page is `docs/index.html`, served by GitHub Pages from `docs/` on `main` at `/<user>.github.io/s3ream/` — all asset paths are relative so the project path works. It is plain HTML + a compiled stylesheet (`docs/assets/landing.css`, built from `docs/src/landing.css` via `pnpm run docs:css` — rerun after touching either, and commit the output); no other build step or CDN dependencies (Inter is self-hosted in `docs/fonts/`). The DaisyUI theme blocks and glass classes mirror `src/renderer/src/index.css` — keep the two files in sync when changing design tokens. The GitHub repo URL appears exactly once, as the `GITHUB_REPO` constant in the bottom inline script of `docs/index.html`; the Pages origin (`https://gr3gg0r.github.io/s3ream/`) is hardcoded in the canonical link, og/twitter URLs, the JSON-LD block, and `docs/sitemap.xml` — all need the same edit if the handle changes.
