...
│   ├── index.html              # Self-contained page; served at /<user>.github.io/S3ream/ (case-sensitive)
...
- The landing page is `docs/index.html`, served by GitHub Pages from `docs/` on `main` at `/<user>.github.io/S3ream/` (Pages project paths are case-sensitive — `/s3ream/` 404s) — all asset paths are relative so the project path works. It is plain HTML + a compiled stylesheet (`docs/assets/landing.css`, built from `docs/src/landing.css` via `pnpm run docs:css` — rerun after touching either, and commit the output); no other build step or CDN dependencies (Inter is self-hosted in `docs/fonts/`). The DaisyUI theme blocks and glass classes mirror `src/renderer/src/index.css` — keep the two files in sync when changing design tokens. The GitHub repo URL appears exactly once, as the `GITHUB_REPO` constant in the bottom inline script of `docs/index.html`; the Pages origin (`https://gr3gg0r.github.io/S3ream/`) is hardcoded in the canonical link, og/twitter URLs, the JSON-LD block, and `docs/sitemap.xml` — all need the same edit if the handle changes. An animated walkthrough of the Simple journey lives in `docs/index.html` (the `[data-demo]` section: class-toggled timeline in the bottom inline script, `.demo-*` styles in `docs/src/landing.css`, static final frame under `prefers-reduced-motion`). `docs/assets/journey-demo.mp4` is a 14.4s capture of one loop, embedded in the README — regenerate by recording the `[data-demo]` element for one cycle (Playwright `recordVideo`, light scheme) and cropping to its bounding box.
